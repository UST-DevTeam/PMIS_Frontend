...
import L2AdvanceFormFORM from "./L2AdvanceFormFORM";
...
const L2AdvanceForm = () => {
  ...
  let dbConfigL = useSelector((state) => {
    let interdata = state?.expenseAdvanceData?.getL2AdvanceData || [""];
    return interdata;
  });
            ...
            child={
              <EditButton
                ...
                onClick={() => {
                  ...
                  setmodalHead("Edit L2 Advance Approval");
                  setmodalBody(
                    <>
                      <L2AdvanceFormFORM
                        isOpen={modalOpen}
                        ...
                        onClick={() => {
                          dispatch(
                            CommonActions.deleteApiCaller(
                              `${Urls.expAdv_L2AdvanceData}/${itm.uniqueId}`,
                              () => {
                                dispatch(
                                  ExpenseAdvanceActions.getL2AdvanceData()
                                );
          ...
          <input
            ...
            onChange={(e) => {
              if (e.target.checked) {
                ...
                setSelectAll(
                  Array.isArray(dbConfigList)
                    ? dbConfigList.map((itm) => itm.uniqueId)
                    : []
                );
                try {
  ...
  function handleAmountAndRemark(type) {
    const data = { approver: "L2-" + type, type: "Advance", status: type };
    const amountRemark = [];
    ...
    selectRows.forEach((key) => {
      ...
      if (originalItem) {
        amountRemark.push({
          _id: key,
          ApprovedAmount:
            key in amount.amount
              ? +amount.amount[key]
              : originalItem?.ApprovedAmount || 0,
          Amount:
            key in amount.claimedAmount
              ? +amount.claimedAmount[key]
              : dbConfigList.find((item) => item.uniqueId === key)?.Amount,
          remark:
            key in amount.remark
              ? amount.remark[key]
              : originalItem?.remark || "Not Mentioned",
          addedFor:
            key in amount.addedFor
              ? amount.addedFor[key]
              : dbConfigList.find((item) => item.uniqueId === key)?.addedFor,
          AdvanceNo:
            key in amount.AdvanceNo
              ? amount.AdvanceNo[key]
              : dbConfigList.find((item) => item.uniqueId === key)?.AdvanceNo,
        });
      }
    ...
    dispatch(
      ExpenseAdvanceActions.postApprovalStatus(true, data, () => {
        dispatch(ExpenseAdvanceActions.getL2AdvanceData({ ids: selectAll }));
  ...
  const onSubmit = (data) => {
    ...
    delete data.reseter;
    dispatch(
      ExpenseAdvanceActions.getL2AdvanceData(value, objectToQueryString(data))
    );
  };
        ...
        headerButton={
          <div className="flex gap-1">
            <Button
              classes="w-auto"
              onClick={(e) => {
                navigate("/home/approverCards/L2Approver");
              }}
              name={"L2 Expense"}
            ></Button>
            <Button
              classes="w-auto"
              onClick={(e) => {
                navigate("/home/approverCards/L2Advance");
              }}
              name={"L2 Advance"}
            ></Button>
            <div className="flex gap-1">
